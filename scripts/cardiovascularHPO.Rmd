...
tmp <- diag(m.commonGenes) %>%
    reshape2::melt() %>%
    `$<-`('HPOTerm', rownames(.)) %>%
    merge(standardNames, by.x='HPOTerm', by.y='HPOTermName', all.x = TRUE, all.y= FALSE, sort=FALSE)
tmp$color <- factor(tmp$query) %>% `levels<-`(groupColors) %>% as.character()
p <- ggplot(tmp) +
    geom_bar(aes(x=factor(HPOTerm, levels=HPOTerm),y=value, fill=query),stat = 'identity') +
    theme_bw() +
