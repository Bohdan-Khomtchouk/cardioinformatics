...
The `associations` table supposedly report one variant per row. However, there are quite a number of row in which variants (SNPs) are concatenated together. There are `r gwas[grepl(pattern = ";", gwas$SNPS),] %>% nrow()` rows of such concatenated values. Below are some examples

```{r}
gwas[grepl(pattern = ";", gwas$SNPS), 'SNPS']
```

Let's check if there are enough information to split these variants into their own rows

```{r}
cols.studies = colnames(gwas.studies)
cols.assoc = colnames(gwas)
```

```{r}
strsplit(gwas$SNPS,split = ';')
```
