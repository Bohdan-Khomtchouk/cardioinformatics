...
\begin{document}
	...
	\author[Khomtchouk \textit{et~al}.]{Bohdan B. Khomtchouk\,$^{\text{\sfb 1,2,3,}*}$, Diem-Trang Tran\,$^{\text{\sfb 4}}$, Or Gozani\,$^{\text{\sfb 1}}$, Themistocles L. Assimes\,$^{\text{\sfb 2, 3}}$}
...
\subsection*{Exploring the role of epigenetics}	
	...
	When more lncRNAs were discovered and characterized, the prevalence of these molecules in cardiovascular biology also emerged. LncRNAs are involved in coronary artery disease, myocardial infarction, cardiac hypertrophy, and more. At least 22 lncRNAs were reportedly dis-regulated in cardiovascular diseases, affecting a wide range of molecular, cellular and physiological processes \citep{Das:2018:Deciphering}. With 107,039 lncRNAs detected in the human genome so far (reported by LNCipedia \citep{Volders:2018:LNCipedia}, as of November 2018), more lncRNAs are likely to be found involved in cardiovascular biology, hence promising therapeutic targets.
	...
	In the Gene Expression Omnibus (GEO), CVD publications have contributed more than 2000 microarray datasets for expression profiling, and about 900 of high-throughput sequencing datasets for various purposes (Figure \ref{fig:geo-assay}). Especially abundant are mRNA-seq assays (\textit{Expression profiling by high-throughput sequencing}), performed as the main assay in most studies, and usually coupled with another profiling technique. In dbGaP where human genotype-phenotype data are deposited, CVD research has collected data on 658305 subjects, 16786 (2.5\%) of whom had consented for the data to be employed for general research (Table \ref{tab:dbgapSubject}). A comprehensive list of CVD studies deposited on dbGaP is available in Supplementary data. \comment{Trang: thinking it's not necessary because these studies can be found on dbGaP browser}
    ...
    Aside from central authoritative repositories of research data, smaller databases with narrower focus are budding. Chromatin structure data from 3C, 4C, 5C and HiC-seq experiments have been collected in dedicated databases such as  3CDB \citep{Yun:2016:3CDB}, 4DGenome \citep{Teng:2015:4DGenome}. Non-coding RNAs are being discovered and added to databases such as lncRNAdb \citep{Quek:2015:lncRNAdb}, NONCODE \citep{Fang:2018:NONCODEV5}, LNCipedia \citep{Volders:2018:LNCipedia}.  Similar to structural variations, data in these under-explored areas are actively collected to enable building reference sets and promise to make the picture of complex diseases more complete.
	
	
	Being dominant in biomedical research are genotype-phenotype data. A list of databases focusing on this type of data was already presented elsewhere \citep{Brookes:2015:Human}. Major projects generating CVD-related omics data are tabulated in table \hl{placeholder}. \comment{discuss about individual vs summary-level data here. explain how these data sets can be used in CVD research}
...
{\begin{tabular}{l l l}
	...
	%%%%%% perspective
Bioinformatics is more than a powerful toolkit for cardiovascular research. Important perspectives have arisen from the application of computational methodology.
For example, when thousand of loci were found to involve in a single disease, the paradigm shifted from additive effect of multiple-genes to the interactions between them, turning the focus from physical genes to the "eigengenes" that represent biologically functional modules \citep{Weiss:2012:Good}. The ability to view and compute on biological system as a whole allows one to focus on the effects of a drug without the mechanistic knowledge of individual actors and their interactions. From the research perspective, such approach is also helpful to understand a system by ways of its response to external stimuli.
