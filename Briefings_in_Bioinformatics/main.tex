...
Our understanding of CVD has been critically limited by our technical capability in characterizing various molecular fractions of a cell. Recent findings gradually revealed more factors contributing to the genetic risk of cardiovascular diseases.

%structural variants
A large fraction of GWA studies are limited to single nuleotide polymorphisms (SNPs), essentially ignoring structural variants including inversion, translocation and copy number variation \comment{cite}. While there are now 660 million of SNPs documented in dbSNP, the number of structural variants in the Database of Genomic Variants Archive (which also includes studies annotated by the NCBI-hosted database of structural variants dbVar) is 4.9 million. Structural variant databases are still the archives of studies instead of data entry for structural variants,
...
% non-coding
When array-based genotyping is gradually replaced by next-generation sequencing, sequencing of exome, i.e. the protein-coding part of the genome, is prioritized over that of the whole genome, based on a regularly cited fact that exome harbors 85\% of the disease-causing variants \citep{Antonarakis:2001:nature}. This figure turns out to be an outdated estimate from 1995. Our survey of all associations in the GWAS catalog \citep{MacArthur:2017:new} ( manually curated to retain only those with p-value less than $10^{-5}$) revealed that a large fraction of variants occur in non-protein coding regions such as intron, interngenic, and splice junctions (Figure \ref{fig:variant_context}). Notably, the distribution of CVD-associated variants mimic that of the complete set of variants.
