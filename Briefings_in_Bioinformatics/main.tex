...
As high-throughput technology was expanded to cover different molecular aspects of a cellular processes, the potential power of integrative analysis on multiple layers of -omics data has been recognized as early as a decade ago \citep{Hawkins:2010:Nextgeneration}. Since then, various strategies have been developed, following one of the two schemes: multi-staged or  \cite{Ritchie:2015:Methods}. In cardiovascular research, multi-staged data integration has been commonly used, for example to match SNPs associated with a given trait to expression quantitative trait loci (eQTL). In these studies, each sample is genotyped and measured for RNA expression levels simultaneously.
 integrated data analysis have become routine, such as the correlation of SNPs with eQTL and phenotype. These multi-staged data integration approaches rely on a  More powerful forms of integration are gaining traction, as the available data become more abundant and heterogeneous.
