...
% non-coding
When array-based genotyping is gradually replaced by next-generation sequencing, sequencing of exome, i.e. the protein-coding part of the genome, is prioritized over that of the whole genome, based on a regularly cited fact that exome harbors 85\% of the disease-causing variants \citep{Antonarakis:2001:nature}. This figure turns out to be an outdated estimate from 1995. Our survey of all associations in the GWAS catalog \citep{MacArthur:2017:new} ( manually curated to retain only those with p-value less than $10^{-5}$) revealed that a large fraction of variants occur in non-protein coding regions such as intronic, interngenic, and splice junctions (Figure \ref{fig:variant_context}). Interestingly, the distribution of CVD-associated variants mimics that of the all-variant set, suggesting that variants accounting for human traits might be distributed in the same manner across the genome.

Recent findings have attributed a more and more important role of epigenetics in cardiovascular disease risk, besides the 


%structural variants
A large fraction of GWA studies are limited to single nuleotide polymorphisms (SNPs), essentially ignoring structural variants including inversion, translocation and copy number variation \comment{cite}. 

chromatin conformation \citep{Rosa-Garrido:2017:HighResolution}

%some focusing on limited area of the human genome
as there is not enough systematic understanding to build their reference database entries. It would be interesting to learn about the occurrence patterns of structural variants, and how they contribute to various phenotypes. Regardless of whether such pattern is shared between the general and clinical populations, it is an under-explored territory that promises more insights toward the long-standing problem of missing heritability  in complex diseases, including CVD.  \comment{More refs. What are the current understanding of SV? How many are there? What are the current challenges of studying SV?}
