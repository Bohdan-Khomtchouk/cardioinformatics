...
submission:
	mkdir -p $(BIBDIR)/figures && cd ${WORKDIR}; make all
	cd ${WORKDIR} && cp main.tex cardio.bib bib.cls main.pdf unsrtnat.bst table*.tex table*.pdf biographical-note.pdf keypoint.pdf OUP_First_SBk_Bot_8401-eps-converted-to.pdf OUP_First_SBk_Bot_8401.eps diff-main.tex diff-main.pdf ../$(BIBDIR)/
