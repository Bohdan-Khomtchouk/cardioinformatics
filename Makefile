...
BIBDIR=bib-submission-r3

submission:
	mkdir -p $(BIBDIR)/figures;
	cp supp/* $(BIBDIR)/;
	cd $(WORKDIR) && cp main.tex cardio.bib bib.cls main.pdf unsrtnat.bst table*.tex table*.pdf tables-editable.xlsx biographical-note.pdf keypoint.pdf OUP_First_SBk_Bot_8401-eps-converted-to.pdf OUP_First_SBk_Bot_8401.eps diff-main.tex diff-main.pdf Makefile ../$(BIBDIR)/
